...
- `GET /burnt` — the memorial list: `[{ number, name, mint, image, traits, lastSeenLeafIndex, burnedAtMs }, ...]` for every Monke ever burnt. Captured live the moment a leaf's DAS entry first flips to `burnt: true` (that page still carries its last-known metadata — we just save it instead of discarding it), so no extra API cost and no external data source needed going forward. A small number of pre-existing burns from before this indexer existed are backfilled from `data/burnt-seed.json` (`burnedAtMs: null` marks those). Purely historical — never re-verified against chain, since a burnt leaf has no live state left to check.
